...
export function defineSeoFields(options = {}) {
    const opts = {
        ...defaultSettings,
        ...options,
    };
    return [
        defineSuperStringField({
            ...
            description: 'Make it as enticing as possible to capture users in Google + social feeds. If it matches the Site Title, only one will be displayed to prevent duplication.',
            options: {
        ...
        defineSuperTextField({
            ...
            validation: createValidationRules(opts.descriptionMinLengthRecommend, opts.descriptionMaxLengthRecommend),
        }),
        defineField({
...
function createValidationRules(minLength, maxLength) {
    return (rule) => [
        ...(minLength
            ? [
                rule
