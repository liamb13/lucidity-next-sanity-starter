 ...
 */
export function onlyWhenParentIs(parentName, parentValue, validator, not = false) {
    return function onlyWhenParentIsValidator(value, context) {
        let shouldValidate = Array.isArray(parentValue)
            ? parentValue.includes(context.parent[parentName])
            : context.parent[parentName] === parentValue;
        if (not) {
 ...
 */
export function requiredIfParentIs(parentName, parentValue, message, not = false) {
    return function requiredIfParentIsValidator(value, context) {
 ...
 */
export function validPostTimestamps(otherFieldName, order, message, thisName, thatName) {
    return function validPostTimestampsValidator(value, context) {
...
export function validUrl(message) {
    return function validUrlValidator(value) {
 ...
 */
export function shouldBeInternalLink(message) {
    return function shouldBeInternalLinkValidator(value) {
        if (typeof value === 'string') {
 ...
 */
export function linkMustBeRelativeOrAbsolute(message) {
    return function linkMustBeRelativeOrAbsoluteValidator(value) {
        if (typeof value === 'string') {
 ...
 */
export function matchesRegex(regex, message) {
    return function matchesRegexValidation(value) {
        if (typeof value === 'string') {
 ...
 */
export function isSlug(message) {
    return matchesRegex(stringIsSlug, message ?? 'Must contain only lowercase letters [a-z], numbers [0-9], and dashes ("-")');
}
